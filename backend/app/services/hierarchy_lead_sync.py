...
``apply_leads_update`` / ``sqlite_row_get`` are satisfied via ``app.core`` helpers
(vl2 does not ship full monolith ``helpers.py`` invariant checks).
"""
...
_COL_SAFE = re.compile(r"^[a-z_][a-z0-9_]*$")
...
def apply_leads_update(
    ...
    *,
    ...
    """Leads UPDATE with IST ``updated_at`` (legacy ``helpers.apply_leads_update`` subset)."""
    _ = log_context
