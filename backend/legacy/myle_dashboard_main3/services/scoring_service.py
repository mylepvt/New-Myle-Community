...
_IDEMPOTENT_PER_LEAD_ACTIONS = frozenset({
    'CALL_ATTEMPT', 'CONNECTED_CALL', 'VIDEO_SENT', 'PAYMENT_DONE',
    'FOLLOWUP_SET', 'FOLLOWUP_COMPLETED',
    'BATCH_MARKED_D1_MORNING', 'BATCH_MARKED_D1_AFTERNOON', 'BATCH_MARKED_D1_EVENING',
    'BATCH_MARKED_D2_MORNING', 'BATCH_MARKED_D2_AFTERNOON', 'BATCH_MARKED_D2_EVENING',
})
...
def add_points(username: str, action_type: str, description: str = "", db=None,
               lead_id: int = 0, **metrics) -> None:
    """
    ...
    try:
        ...
        import logging
        logging.getLogger('scoring').error(f"Error adding points for {username}: {e}")
    finally:
        if close_db:
            _db.close()
...
def apply_penalty(username: str, penalty_action: str = 'FOLLOWUP_MISSED', description: str = "", db=None) -> None:
    """
...
def check_progression(username: str, db=None) -> None:
    ...
    try:
        user = _db.execute("SELECT user_stage, total_points FROM users WHERE username = ?", (username,)).fetchone()
        if not user:
            return
        ...
        if current_stage == 'day1':
            res = _db.execute("SELECT COUNT(*) as count FROM point_history WHERE username = ? AND action_type = 'FOLLOWUP_COMPLETED'", (username,)).fetchone()
        ...
        import logging
        logging.getLogger('scoring').error(f"Error checking progression for {username}: {e}")
    finally:
        if close_db:
            _db.close()
